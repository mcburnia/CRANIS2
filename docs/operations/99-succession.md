...
> **For:** The executors, heirs, or designated successors of Andrew (Andi) MCBURNIE — the founder, sole owner, and operator of CRANIS2.
> **Open this if:** Andi is no longer available — through death, long-term incapacity, or absence — and you need to take responsibility for CRANIS2.
> **Otherwise:** Do not act on this document. It is a contingency, not an operating procedure.

> **Important context:** CRANIS2 is, at the time of writing, owned and operated by Andi personally. There is no limited company behind it. Any commercial vehicle (e.g. a UK Ltd) intended to receive seed investment will be set up at the appropriate moment; if that has happened by the time you read this, the relevant company paperwork supersedes the personal-ownership references below — but the operational guidance still applies.

---

## A note from Andi

> *[FILL IN: Personal note from Andi to his sons. This section is intentionally left for Andi to write in his own words. It might cover: why CRANIS2 matters, what he hopes you'll do, anything personal he wants you to read at this moment. Keep separate from the operational content below — that part stays factual and doesn't need a personal voice.]*
...
3. **Do not cancel Andi's bank cards or any payment cards immediately.** Several services renew automatically (the domain, the VPS, Cloudflare, Stripe payouts, email sending, Anthropic). If those payments fail, the platform breaks. The first job is to transfer billing authority into a successor account, not to stop it.
4. **Do not announce anything to customers** until you have read sections 1–4 below and understand what you are saying.
5. **Do not sell, transfer, or wind down the operation in the first 30 days.** You almost certainly do not have all the information you need yet, and any of those decisions are reversible only at significant cost.
...
2. **Customer data stays safe.** No deletion, no leak, no unauthorised access. The platform handles compliance evidence — losing it has reputational and legal consequences far beyond any one customer.
3. **The operation stays solvent.** Bills get paid; statutory filings (where applicable) happen on time; any contractor or service provider currently engaged gets paid.
4. **Your own wellbeing.** Take breaks. Sleep. This is a marathon, not a sprint.

Anything else (new features, marketing, partnerships, Andi's roadmap) waits until those four are stable.
...
1. **A USB stick in Andi's safe.** Location of the safe: *[FILL IN: e.g. "the office at home, top right drawer of the filing cabinet, combination on a card in [father-in-law]'s safe"]*. Combination of the safe: *[FILL IN: where to find this — do NOT write the combination into this document]*.
2. **A sealed envelope held by Andi's solicitor.** Solicitor name: *[FILL IN]*. Firm: *[FILL IN]*. Phone: *[FILL IN]*. The envelope contains a printed copy of `OPERATIONS-SECURE.md` plus the age decryption key for the encrypted backup mirror.
3. **Andi's password manager.** Vault provider: *[FILL IN: e.g. "Bitwarden"]*. The master password is shared with you separately — see *[FILL IN: e.g. "the sealed letter Andi gave you alongside this document"]*.

If you cannot get into any of those three sources, contact the solicitor first (they have a sealed copy and a duty to release it on production of a death certificate or appropriate authority).
...
2. **Get into the dev server.** This is the Mac Mini at Andi's home address. SSH access details are in `OPERATIONS-SECURE.md`. From the dev server you can read everything in this repository and the operational scripts.
...
4. **Speak to Andi's solicitor and accountant.** Both will have specific advice about the estate's position and your obligations. Names and contact details in `OPERATIONS-SECURE.md`. If a UK limited company has been incorporated by the time you read this, the solicitor and accountant will also advise on its status.
5. **Do not announce anything publicly yet.** Continue routine operations. If a customer raises an issue, respond as the platform has always responded — you do not need to disclose the change in operatorship while you are still establishing what is happening.
...
|---|---|---|
| **Continue** | You and your brother run the operation between you, possibly with help from a hired technical lead. | You both have time and willingness; the financials look workable; you can hire to cover the technical gap. |
| **Sell** | You find a buyer — likely a CRA-adjacent compliance vendor or a consultancy — and transfer the operation (or the company, if one has been incorporated by then). | You don't have time, but there are positive financials and customer goodwill. |
| **Wind down** | You give customers notice, return data, refund where appropriate, and close the operation in good order. | The financials don't work, or no buyer can be found, or you both decide it isn't the right path. |
...
| Accountant | Personal and (if incorporated) company finances, statutory filings, VAT, payroll | `OPERATIONS-SECURE: accountant` |
| Bank — relationship manager | Transfer signatory authority on the operating account | `OPERATIONS-SECURE: bank.contact` |
| Insurance broker | Notify of change of operator, confirm policy validity | `OPERATIONS-SECURE: insurance` |
...
| Data protection supervisor | Whichever supervisory authority CRANIS2 is registered with at the time you read this | `OPERATIONS-SECURE: data-protection` |

---

## What you must not do, ever

These rules are absolute. They protect customers and the operation. Breaking any one of them creates either a regulatory incident or an unrecoverable data loss.
...
*Last updated: 2026-05-02 (rewritten as a personal succession plan to reflect that CRANIS2 is owned and operated by Andrew (Andi) MCBURNIE personally; awaiting Andi's fill-in of the personal note and the contact details that live in `OPERATIONS-SECURE.md`).*
