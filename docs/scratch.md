...
- ** 104 feat(feedback-notify): email `support@cranis2.com` whenever a customer submits via the in-app FeedbackModal (`POST /api/feedback`), across all three categories (`feedback`, `bug`, `feature`). Mirror the existing admin-notify pattern that already emails `info@cranis2.com` on signup (`a9746b3`, `478253c`). Subject encodes category + subject (e.g. `[CRANIS2 BUG] Sync button greyed out on Codeberg repos`). Body must include: category chip; submitter name + email + organisation + `org_role` + billing plan + trial/active state; the `pageUrl` they were on; subject (verbatim); body text (verbatim, preserving newlines); UTC + locale timestamps; deep-link to `/admin/users/:userId`; `mailto:` reply link. Destination configurable via `SUPPORT_NOTIFY_EMAIL` env var (default `support@cranis2.com`). Send failures must NOT block the API response — log and continue. Suppressed in test mode (`CRANIS2_TEST_MODE=true`). Tests: unit on the formatter, integration that POST /api/feedback triggers exactly one (mocked) email send with the correct payload, integration that response is still 200 when the email client throws. Out of scope: two-way reply threading, ticket-system sync, status-change notifications, digest mode (every submission gets its own email). Prereq (user-side): the `support@cranis2.com` mailbox must be provisioned under the cranis2.com domain before this ships **— DONE 2026-05-18**. Estimated effort: small (~½ day).
