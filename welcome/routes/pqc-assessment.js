...
const { SECTIONS, READINESS_LABELS } = require('../data/pqc-questions');
const { computeScores, determineReadinessLevel, getReadinessDetails, getTopRecommendations } = require('../lib/pqc-scoring');
const { buildReportEmail } = require('../templates/pqc-report-email');
const { assessmentPage } = require('../templates/pqc-page');
...
router.get('/', (req, res) => {
  logAccess(req, 'pqc_assessment_tool');
...
router.post('/send-code', async (req, res) => {
  ...
  try {
    ...
    if (!isConfigured()) {
      console.log(`[DEV] PQC verification code for ${email}: ${code}`);
      return res.json({ ok: true, dev_code: code });
    }

    const emailRes = await sendVerificationCode(email, code, 'PQC');
    if (!emailRes.ok) {
      return res.status(502).json({ error: 'Failed to send verification email.' });
    }

    logAccess(req, 'pqc_code_sent');
    res.json({ ok: true });
  } catch (err) {
    console.error('PQC send code error:', err);
...
router.post('/verify', async (req, res) => {
  ...
  try {
    if (skipCode) {
      ...
      await markEmailVerified(email, 'pqc');
    }

    const existing = await pool.query(
      `SELECT id, answers, current_section, completed_at FROM pqc_assessments
    ...
    if (existing.rows.length > 0 && !existing.rows[0].completed_at) {
      ...
      const newAssessment = await pool.query(
        `INSERT INTO pqc_assessments (email) VALUES ($1) RETURNING id, answers, current_section`,
        [email.toLowerCase()]
      );
    ...
    if (isNewAssessment && isConfigured()) {
      sendLeadNotification({
        subject: `New PQC assessment started \u2013 ${email.toLowerCase()}`,
        html: `<p>A new user has started the PQC Readiness Assessment:</p><p><strong>${escapeHtml(email)}</strong></p><p>Assessment ID: ${assessment.id}</p><p>Time: ${new Date().toISOString()}</p>`,
      });
    }

    logAccess(req, 'pqc_verified');
    res.json({
      ok: true,
    ...
    console.error('PQC verify code error:', err);
...
router.post('/save-progress', async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE pqc_assessments SET answers = $1, current_section = $2, updated_at = NOW()
       WHERE id = $3 AND completed_at IS NULL`,
      [JSON.stringify(answers || {}), currentSection || 0, assessmentId]
    );
    res.json({ ok: true });
  } catch (err) {
    console.error('PQC save progress error:', err);
...
router.post('/complete', async (req, res) => {
  ...
  try {
    ...
    const readinessLevel = determineReadinessLevel(scores);

    await pool.query(
      `UPDATE pqc_assessments
       SET answers = $1, scores = $2, readiness_level = $3, completed_at = NOW(), updated_at = NOW(),
           current_section = $4
       WHERE id = $5`,
      [JSON.stringify(answers || {}), JSON.stringify(scores), readinessLevel, SECTIONS.length, assessmentId]
    );

    logAccess(req, 'pqc_completed');
    res.json({ ok: true, scores, readinessLevel });
  } catch (err) {
    console.error('PQC complete assessment error:', err);
...
router.post('/send-report', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT answers, scores, readiness_level, email FROM pqc_assessments WHERE id = $1`,
      [assessmentId]
    );
    ...
    const { answers, scores, readiness_level } = result.rows[0];
    const readinessDetails = getReadinessDetails(readiness_level);
    const recommendations = getTopRecommendations(scores, answers);

    const reportHtml = buildReportEmail(answers, scores, readiness_level, readinessDetails, recommendations, escapeHtml, getUnsubscribeUrl);

    const emailRes = await sendEmail({
      from: 'CRANIS2 <noreply@poste.cranis2.com>',
      to: reportEmail,
      subject: `Your PQC Readiness Assessment Report \u2013 ${scores.overallPct}% Ready`,
      html: reportHtml,
    });
    ...
    await sendEmail({
      ...
      to: 'info@cranis2.com',
      subject: `PQC Assessment Completed \u2013 ${reportEmail} (${scores.overallPct}% ready, ${READINESS_LABELS[readiness_level]})`,
      html: `<h3>PQC Readiness Assessment Completed</h3>
...
<tr><td style="padding:4px 12px 4px 0;font-weight:bold;">Level</td><td>${READINESS_LABELS[readiness_level]}</td></tr>
</table>
...
${SECTIONS.map(s => `<tr><td style="padding:2px 12px 2px 0;">${s.title}</td><td>${scores.sections[s.id].pct}% (${scores.sections[s.id].level})</td></tr>`).join('\n')}
    ...
    logAccess(req, 'pqc_report_sent');
    res.json({ ok: true });
  } catch (err) {
    console.error('PQC send report error:', err);
